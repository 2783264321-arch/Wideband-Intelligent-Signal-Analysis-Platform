...
export function BenchmarkCreatePanel({ onCreated }: { onCreated: (id: string) => void }) {
  ...
  return (
    <Space direction="vertical" style={{ width: "100%" }}>
      ...
      <Select
        aria-label={t("benchmarks.importedBatch")}
        value={fingerprint}
        placeholder={t("benchmarks.selectImportedBatch")}
        onChange={(value) => { setFingerprint(value); setResolution(undefined); }}
        options={batches.map((batch) => ({
          value: batch.importFingerprint,
          disabled: !batch.ready,
          label: batch.ready
            ? t("benchmarks.batchOption", {
                pipeline: batch.pipelineId,
                dataset: batch.datasetName,
                split: batch.datasetSplit,
                count: batch.runCount,
              })
            : t("benchmarks.batchInvalid", { fingerprint: batch.importFingerprint.slice(0, 12) }),
        }))}
        style={{ width: "100%" }}
      />
      <Descriptions column={1} size="small">
        <Descriptions.Item label={t("benchmarks.evaluationProtocol")}>
          <Typography.Text code>physical_tf_detection_ap_v2</Typography.Text>
        </Descriptions.Item>
        <Descriptions.Item label={t("benchmarks.gtPolicy")}>{t("benchmarks.gtPolicyValue")}</Descriptions.Item>
        {resolution ? <>
          <Descriptions.Item label={t("benchmarks.resolved")}>{resolution.resolvedRecordings} / {resolution.expectedRecordings}</Descriptions.Item>
          <Descriptions.Item label={t("benchmarks.missing")}>{resolution.missingRecordings}</Descriptions.Item>
          <Descriptions.Item label={t("benchmarks.conflicts")}>{resolution.conflictCount}</Descriptions.Item>
          <Descriptions.Item label={t("benchmarks.manifestSha")}>{resolution.recordingManifestHash}</Descriptions.Item>
        </> : null}
      </Descriptions>
      <Button onClick={() => void resolve()} disabled={!fingerprint} loading={busy}>{t("benchmarks.resolve")}</Button>
      <Input aria-label={t("benchmarks.benchmarkName")} placeholder={t("benchmarks.benchmarkName")} value={benchmarkName} onChange={(e) => setBenchmarkName(e.target.value)} />
      <Button type="primary" onClick={() => void createAndRun()} disabled={!readyToCreate} loading={busy}>{t("benchmarks.createAndRun")}</Button>
    </Space>
  );
}